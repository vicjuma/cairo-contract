...
pub trait INFT<TContractState> {
    ...
    fn purchase_policy(
        ref self: TContractState, coverage_type: CoverageType, amount: u256, data: Span<felt252>, descrption: ByteArray,
    ) -> felt252;
    ...
    fn approve_tokens(
        ref self: TContractState, token_address: ContractAddress, spender: ContractAddress, amount: u256,
    ) -> bool;
...
pub mod PolicyNFTComponent {
    ...
    use core::{hash::{HashStateTrait}, poseidon::{PoseidonTrait}};
    use openzeppelin::access::ownable::OwnableComponent;
    use openzeppelin::introspection::src5::SRC5Component;
    use openzeppelin::security::{ReentrancyGuardComponent};
    use openzeppelin::token::{
        erc20::{ERC20ABIDispatcher, ERC20ABIDispatcherTrait}, erc721::{ERC721Component, ERC721HooksEmptyImpl},
    };
    ...
    use starknet::storage::{
        Map, MutableVecTrait, StoragePathEntry, StoragePointerReadAccess, StoragePointerWriteAccess, Vec,
    };
    use starknet::{SyscallResultTrait, get_block_timestamp, get_caller_address, get_contract_address, syscalls};
    use super::{AssertedPolicy, ContractAddress, CoverageType, DisputedClaim, INFT, Policy, contract_address_const};

    pub mod Errors {
        pub const UNSUCCESSFUL_PAYMENT: felt252 = 'Payment Was Not Successful!';
        pub const CANNOT_DISPUTE_YOUR_CLAIM: felt252 =
            'NOT Authorized to Dispute'; // should be not authorized to dispute
    ...
    #[derive(Drop, starknet::Event)]
    pub struct PolicyDisputed {
        pub dispute_id: felt252,
        pub policy_id: felt252,
        #[key]
        pub assertion_id: felt252,
    ...
    > of INFT<ComponentState<TContractState>> {
        ...
        fn safe_mint(
            ...
            // let mut ownable =
            // get_dep_component_mut!(ref self,
            // Ownable);
            let mut erc721 = get_dep_component_mut!(ref self, ERC721);
            let int_token_id: u256 = token_id.try_into().unwrap();
        ...
        fn safeMint(
            ref self: ComponentState<TContractState>, recipient: ContractAddress, tokenId: felt252, data: Span<felt252>,
        ) {
        ...
        fn purchase_policy(
            ...
            let policy_id = self
                .generate_unique_id(policyholder.try_into().unwrap(), get_block_timestamp().try_into().unwrap());
            ...
            let mut policyholder_policies = self
                .policyholder_policies
                .entry(policyholder); // returns a pointer thus no need to read()
        ...
        fn file_claim(
            ref self: ComponentState<TContractState>, policy_id: felt252, claim_description: ByteArray,
        ) -> felt252 {
            let erc20 = ERC20ABIDispatcher {
                contract_address: 0x049d36570d4e46f48e99674bd3fcc84644ddd6b96f7c741b1562b82f9e004dc7
                    .try_into()
            ...
            let businessArr: ByteArray =
                "Business operations was fully or partially interrupted for a specified period due to the pandemic causing loss of income";
            let individualArr: ByteArray =
                "Myself or anyone close to me experienced a financial or personal loss or health deterioration due to the pandemic.";
            let eventArr: ByteArray =
                "The scheduled event was fully or partially canceled due to the pandemic causing loss of income";
            let claims_array = array![
                self.hash_byte_array(@businessArr),
                self.hash_byte_array(@individualArr),
                self.hash_byte_array(@eventArr),
            ];
            ...
            match policy_clone.coverageType {
                CoverageType::BusinessInterruptions => assert(
                    self.hash_byte_array(@claim_description) == business, Errors::INVALID_BUSINESS_COVERAGE,
                ),
                CoverageType::IndividualCoverage => assert(
                    self.hash_byte_array(@claim_description) == individual, Errors::INVALID_INDIVIDUAL_COVERAGE,
                ),
                CoverageType::EventCancellations => assert(
                    self.hash_byte_array(@claim_description) == event, Errors::INVALID_EVENT_COVERAGE,
                ),
            ...
            let claim = self
                .compose_claim(claim_description.clone(), policy_clone.coverageType, policy_clone.description);
            // let success =
            // erc20.transfer_from(asserter,
            // get_contract_address(), bond);
            // assert(success,
            // Errors::BOND_TRANSFER_FAILD);
            ...
            self
                .asserted_policies
                .entry(assertion_id)
                .write(AssertedPolicy { assertion_id: assertion_id, policy_id: policy_id, asserter: asserter });
            assertion_id
        }
        ...
        fn dispute_asserted_claim(ref self: ComponentState<TContractState>, assertion_id: felt252) -> felt252 {
            ...
            let dispute_id: felt252 = PoseidonTrait::new()
                .update(assertion_id)
                .update(disputer.try_into().unwrap())
                .finalize();
            ...
            self
                .disputed_policies
                .entry(assertion_id)
                .write(DisputedClaim { dispute_id, policy_id, asserter, disputer });
        ...
        fn resolve_dispute(ref self: ComponentState<TContractState>, assertion_id: felt252) -> bool {
            ...
            if !is_disputed || result {
                ...
                // assertion_id, result: false });
                false
            }
        }

        fn push_price(ref self: ComponentState<TContractState>, assertion_id: felt252, price: u256) {
            let contract_address: ContractAddress = 0xb7940968024a3f9963968226e07904343ba95f2773dadc2fb98863430f1187
                .try_into()
                .unwrap();
            ...
            let _res = syscalls::call_contract_syscall(
                contract_address, selector!("push_price_by_request_id"), call_data.span(),
            )
                .unwrap_syscall();
        }
        ...
        fn calculate_premium(ref self: ComponentState<TContractState>, coverage: CoverageType) -> u256 {
            ...
            match coverage {
                CoverageType::BusinessInterruptions => (2400 * self.calculate_amount(10, 8))
                    / conv.get_eth_to_usd().try_into().unwrap(),
                CoverageType::EventCancellations => (6000 * self.calculate_amount(10, 8))
                    / conv.get_eth_to_usd().try_into().unwrap(),
                CoverageType::IndividualCoverage => (600 * self.calculate_amount(10, 8))
                    / conv.get_eth_to_usd().try_into().unwrap(),
            }
        }
        ...
        fn dollar_to_wei(ref self: ComponentState<TContractState>, assertion_fee: u128) -> u128 {
            ...
            (assertion_fee
                * self.calculate_amount(10, conv.get_decimals_eth()).try_into().unwrap()
                * 1000000000000000000)
                / (price_in_usd * self.calculate_amount(10, conv.get_decimals_eth()).try_into().unwrap())
        }
        ...
        fn approve_tokens(
            ...
            let mut res = syscalls::call_contract_syscall(token_address, selector!("approve"), call_data.span())
                .unwrap_syscall();
    ...
    > of INFTInternal<TContractState> {
        fn initializer(ref self: ComponentState<TContractState>, owner: ContractAddress) {
            ...
            erc721
                .initializer(
                    "SafeHaven",
                    "FH",
                    "https://aquamarine-holy-mosquito-124.mypinata.cloud/files/bafybeifke37iyx2ozevnqmck3uhpnocij4nsltjq4og7utioh55mhdnuvu?X-Algorithm=PINATA1&X-Date=1736404721&X-Expires=30&X-Method=GET&X-Signature=26e433ef5af65b140719afcb8bdbda825a4085e577d6fcd531af891c9097cd7b",
                );
        ...
        fn generate_unique_id(
            ref self: ComponentState<TContractState>, user_address: felt252, timestamp: felt252,
        ) -> felt252 {
            let hash = PoseidonTrait::new().update(user_address).update(timestamp).finalize();
            hash
        }
        ...
        // fn dollar_to_wei(ref self:
    // ComponentState<TContractState>,
    // assertion_fee: u128) -> u128 {
    //     let mut conv =
    //     get_dep_component_mut!(ref self,
    //     converter);
    //     let price_in_usd =
    //     conv.get_eth_to_usd();
    //     (assertion_fee *
    //     self.calculate_amount(10,
    //     conv.get_decimals_eth()).try_into().unwrap()
    //     * 1000000000000000000) /
    //     (price_in_usd *
    //     self.calculate_amount(10,
    //     conv.get_decimals_eth()).try_into().unwrap())
